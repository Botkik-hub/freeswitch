...
static switch_status_t load_tiers(switch_bool_t load_all, const char *queue_name, const char *agent_name,
								  switch_event_t *params, switch_xml_t x_tiers_cfg);
...
static struct cc_state_table STATE_CHART[] = {{"Unknown", CC_TIER_STATE_UNKNOWN},
...
static struct cc_status_table AGENT_STATUS_CHART[] = {{"Unknown", CC_AGENT_STATUS_UNKNOWN},
...
static struct cc_state_table AGENT_STATE_CHART[] = {{"Unknown", CC_AGENT_STATE_UNKNOWN},
...
static struct cc_state_table MEMBER_STATE_CHART[] = {{"Unknown", CC_MEMBER_STATE_UNKNOWN},
...
typedef enum { CC_APP_AGENT_CONNECTING = (1 << 0), CC_APP_SHOULD_BREAK = (1 << 1) } cc_app_flag_t;

static char members_sql[] = "CREATE TABLE members (\n"
							"   queue	     VARCHAR(255),\n"
							"   instance_id	     VARCHAR(255),\n"
							"   uuid	     VARCHAR(255) DEFAULT '' NOT NULL,\n"
							"   session_uuid     VARCHAR(255) DEFAULT '',\n"
...
static char agents_sql[] = "CREATE TABLE agents (\n"
						   "   name      VARCHAR(255),\n"
						   "   instance_id    VARCHAR(255),\n"
						   "   uuid      VARCHAR(255),\n"
						   "   type      VARCHAR(255),\n" /* Callback , Dial in...*/
...
static char tiers_sql[] = "CREATE TABLE tiers (\n"
						  ...
						  /*
							 ...
							 Offering
						   */
						  "   col_level    INTEGER DEFAULT 1 NOT NULL,\n"
...
cc_member_cancel_reason_t cc_member_cancel_str2reason(const char *str)
{
	...
	for (x = 0; x < (sizeof(MEMBER_CANCEL_REASON_CHART) / sizeof(struct cc_member_cancel_reason_table)) - 1 &&
				MEMBER_CANCEL_REASON_CHART[x].name;
		 x++) {
...
cc_agent_status_t cc_agent_str2status(const char *str)
{
	...
	for (x = 0; x < (sizeof(AGENT_STATUS_CHART) / sizeof(struct cc_status_table)) - 1 && AGENT_STATUS_CHART[x].name;
		 x++) {
...
cc_member_state_t cc_member_str2state(const char *str)
{
	...
	for (x = 0; x < (sizeof(MEMBER_STATE_CHART) / sizeof(struct cc_state_table)) - 1 && MEMBER_STATE_CHART[x].name;
		 x++) {
...
typedef enum { PFLAG_DESTROY = 1 << 0 } cc_flags_t;
...
static void destroy_queue(const char *queue_name)
{
	...
	if (switch_thread_rwlock_trywrlock(queue->rwlock) != SWITCH_STATUS_SUCCESS) {
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "[%s] queue is in use, memory will be freed whenever its no longer in use\n", queue->name);
		return;
	}
...
switch_cache_db_handle_t *cc_get_db_handle(void)
{
	...
	if (switch_cache_db_get_db_handle_dsn(&dbh, dsn) != SWITCH_STATUS_SUCCESS) { dbh = NULL; }
...
cc_queue_t *queue_set_config(cc_queue_t *queue)
{
	 ...
	 */
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "strategy", SWITCH_CONFIG_STRING, 0, &queue->strategy,
						   "longest-idle-agent", &queue->config_str_pool, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "moh-sound", SWITCH_CONFIG_STRING, 0, &queue->moh, NULL,
						   &queue->config_str_pool, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "announce-sound", SWITCH_CONFIG_STRING, 0, &queue->announce, NULL,
						   &queue->config_str_pool, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "announce-frequency", SWITCH_CONFIG_INT, 0, &queue->announce_freq, 0,
						   &config_int_0_86400, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "record-template", SWITCH_CONFIG_STRING, 0, &queue->record_template,
						   NULL, &queue->config_str_pool, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "time-base-score", SWITCH_CONFIG_STRING, 0, &queue->time_base_score,
						   "queue", &queue->config_str_pool, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "ring-progressively-delay", SWITCH_CONFIG_INT, 0,
						   &queue->ring_progressively_delay, NULL, &config_int_0_86400, NULL, NULL);

	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "tier-rules-apply", SWITCH_CONFIG_BOOL, 0, &queue->tier_rules_apply,
						   SWITCH_FALSE, NULL, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "tier-rule-wait-second", SWITCH_CONFIG_INT, 0,
						   &queue->tier_rule_wait_second, 0, &config_int_0_86400, NULL, NULL);
	...
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "tier-rule-no-agent-no-wait", SWITCH_CONFIG_BOOL, 0,
						   &queue->tier_rule_no_agent_no_wait, SWITCH_TRUE, NULL, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "discard-abandoned-after", SWITCH_CONFIG_INT, 0,
						   &queue->discard_abandoned_after, 60, &config_int_0_86400, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "abandoned-resume-allowed", SWITCH_CONFIG_BOOL, 0,
						   &queue->abandoned_resume_allowed, SWITCH_FALSE, NULL, NULL, NULL);

	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "max-wait-time", SWITCH_CONFIG_INT, 0, &queue->max_wait_time, 0,
						   &config_int_0_86400, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "max-wait-time-with-no-agent", SWITCH_CONFIG_INT, 0,
						   &queue->max_wait_time_with_no_agent, 0, &config_int_0_86400, NULL, NULL);
	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "max-wait-time-with-no-agent-time-reached", SWITCH_CONFIG_INT, 0,
						   &queue->max_wait_time_with_no_agent_time_reached, 5, &config_int_0_86400, NULL, NULL);

	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "agent-no-answer-status", SWITCH_CONFIG_STRING, 0,
						   &queue->agent_no_answer_status, cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK),
						   &queue->config_str_pool, NULL, NULL);

	SWITCH_CONFIG_SET_ITEM(queue->config[i++], "skip-agents-with-external-calls", SWITCH_CONFIG_BOOL, 0,
						   &queue->skip_agents_with_external_calls, SWITCH_TRUE, NULL, NULL, NULL);
...
static switch_bool_t cc_execute_sql_callback(cc_queue_t *queue, switch_mutex_t *mutex, char *sql,
							  ...
							  switch_xml_t x_queues_cfg)
{
	...
	if (x_queues_cfg) {
		...
		if (!(x_queues = switch_xml_child(cfg, "queues"))) { goto end; }
	}
	...
	if ((x_queue = switch_xml_find_child(x_queues, "queue", "name", queue_name))) {
		...
		if (cc_agent_str2status(queue->agent_no_answer_status) == CC_AGENT_STATUS_UNKNOWN) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
							  "Queue %s has invalid agent-no-answer-status, setting to %s", queue->name,
							  cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK));
	...
	if (queue && request_agents && (x_agents = switch_xml_child(cfg, "agents"))) {
		for (x_agent = switch_xml_child(x_agents, "agent"); x_agent; x_agent = x_agent->next) {
			...
			if (agent) { load_agent(agent, params, x_agents); }
		}
	}
	...
	if (!switch_strlen_zero(queue)) {
		if (queue[0] == '*') {
			...
			sql = switch_mprintf("SELECT count(*) FROM members WHERE queue = '%q' AND (state = '%q' OR state = '%q')",
								 queue, cc_member_state2str(CC_MEMBER_STATE_WAITING),
								 cc_member_state2str(CC_MEMBER_STATE_TRYING));
		}
...
cc_status_t cc_agent_add(const char *agent, const char *type)
{
	...
	if (!strcasecmp(type, CC_AGENT_TYPE_CALLBACK) || !strcasecmp(type, CC_AGENT_TYPE_UUID_STANDBY)) {
		...
		sql = switch_mprintf(
			"INSERT INTO agents (name, instance_id, type, status, state) VALUES('%q', 'single_box', '%q', '%q', '%q');",
	...
	if (!strcasecmp(key, "status")) {
		if (cc_agent_str2status(value) != CC_AGENT_STATUS_UNKNOWN) {
			...
			if (cc_agent_str2status(value) == CC_AGENT_STATUS_AVAILABLE) {
				sql = switch_mprintf("UPDATE agents SET status = '%q', last_status_change = '%" SWITCH_TIME_T_FMT
									 "', talk_time = 0, calls_answered = 0, no_answer_count = 0"
									 " WHERE name = '%q' AND NOT status = '%q'",
									 value, local_epoch_time_now(NULL), agent, value);
			} else {
				sql = switch_mprintf("UPDATE agents SET status = '%q', last_status_change = '%" SWITCH_TIME_T_FMT
									 "' WHERE name = '%q'",
			...
			if (cc_agent_str2status(value) != CC_AGENT_STATUS_AVAILABLE) {
				sql = switch_mprintf("SELECT uuid FROM members WHERE serving_agent = '%q' AND serving_system = "
									 "'single_box' AND NOT state = 'Answered'",
									 agent);
				cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
				switch_safe_free(sql);
				if (!switch_strlen_zero(res)) {
					switch_core_session_hupall_matching_var("cc_member_pre_answer_uuid", res,
		...
		if (cc_agent_str2state(value) != CC_AGENT_STATE_UNKNOWN) {
			if (cc_agent_str2state(value) != CC_AGENT_STATE_RECEIVING) {
				...
				sql = switch_mprintf("UPDATE agents SET state = '%q', last_offered_call = '%" SWITCH_TIME_T_FMT
									 "' WHERE name = '%q'",
									 value, local_epoch_time_now(NULL), agent);
			}
			cc_execute_sql(NULL, sql, NULL);
		...
		sql =
			switch_mprintf("UPDATE agents SET uuid = '%q', instance_id = 'single_box' WHERE name = '%q'", value, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);

		result = CC_STATUS_SUCCESS;
	} else if (!strcasecmp(key, "contact")) {
		sql = switch_mprintf("UPDATE agents SET contact = '%q', instance_id = 'single_box' WHERE name = '%q'", value,
							 agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE agents SET ready_time = '%ld', instance_id = 'single_box' WHERE name = '%q'",
							 atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE agents SET busy_delay_time = '%ld', instance_id = 'single_box' WHERE name = '%q'",
							 atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql =
			switch_mprintf("UPDATE agents SET reject_delay_time = '%ld', instance_id = 'single_box' WHERE name = '%q'",
						   atol(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf(
			"UPDATE agents SET no_answer_delay_time = '%ld', instance_id = 'single_box' WHERE name = '%q'", atol(value),
			agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE agents SET max_no_answer = '%d', instance_id = 'single_box' WHERE name = '%q'",
							 atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE agents SET wrap_up_time = '%d', instance_id = 'single_box' WHERE name = '%q'",
							 atoi(value), agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		if (cc_agent_str2state(value) == CC_AGENT_STATE_UNKNOWN) {
			...
			sql = switch_mprintf(
				"UPDATE agents SET state = '%q' WHERE name = '%q' AND state = '%q' AND status IN ('%q', '%q')", value,
				agent, cc_agent_state2str(CC_AGENT_STATE_WAITING), cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE),
				cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE_ON_DEMAND));

			if (cc_execute_sql_affected_rows(sql) > 0) {
				result = CC_STATUS_SUCCESS;
				if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CALLCENTER_EVENT) ==
...
cc_status_t cc_tier_add(const char *queue_name, const char *agent, const char *state, int level, int position)
{
	...
	if (cc_tier_str2state(state) != CC_TIER_STATE_UNKNOWN) {
		...
		/* Add Agent in tier */
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "Adding Tier on Queue %s for Agent %s, col_level %d, position %d\n", queue_name, agent, level,
						  position);
		sql = switch_mprintf(
			"INSERT INTO tiers (queue, agent, state, col_level, position) VALUES('%q', '%q', '%q', '%d', '%d');",
...
cc_status_t cc_tier_update(const char *key, const char *value, const char *queue_name, const char *agent)
{
	...
	if (!strcasecmp(key, "state")) {
		if (cc_tier_str2state(value) != CC_TIER_STATE_UNKNOWN) {
			sql = switch_mprintf("UPDATE tiers SET state = '%q' WHERE queue = '%q' AND agent = '%q'", value, queue_name,
								 agent);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
			result = CC_STATUS_SUCCESS;
	...
	} else if (!strcasecmp(key, "col_level")) {
		sql = switch_mprintf("UPDATE tiers SET col_level = '%d' WHERE queue = '%q' AND agent = '%q'", atoi(value),
							 queue_name, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		...
		sql = switch_mprintf("UPDATE tiers SET position = '%d' WHERE queue = '%q' AND agent = '%q'", atoi(value),
							 queue_name, agent);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
...
static switch_status_t load_agent(const char *agent_name, switch_event_t *params, switch_xml_t x_agents_cfg)
{
	...
	if (x_agents_cfg) {
		...
		if (!(x_agents = switch_xml_child(cfg, "agents"))) { goto end; }
	}
	...
	if ((x_agent = switch_xml_find_child(x_agents, "agent", "name", agent_name))) {
		...
		if (type) {
			...
			if (res == CC_STATUS_SUCCESS || res == CC_STATUS_AGENT_ALREADY_EXIST) {
				if (contact) { cc_agent_update("contact", contact, agent_name); }
				if (status) { cc_agent_update("status", status, agent_name); }
				if (wrap_up_time) { cc_agent_update("wrap_up_time", wrap_up_time, agent_name); }
				if (max_no_answer) { cc_agent_update("max_no_answer", max_no_answer, agent_name); }
				if (reject_delay_time) { cc_agent_update("reject_delay_time", reject_delay_time, agent_name); }
				if (busy_delay_time) { cc_agent_update("busy_delay_time", busy_delay_time, agent_name); }
				if (no_answer_delay_time) { cc_agent_update("no_answer_delay_time", no_answer_delay_time, agent_name); }

				if (type && res == CC_STATUS_AGENT_ALREADY_EXIST) { cc_agent_update("type", type, agent_name); }
			}
		}
	}
...
static switch_status_t load_tier(const char *queue, const char *agent, const char *level, const char *position)
{
	...
	if (cc_tier_update("unknown", "unknown", queue, agent) == CC_STATUS_TIER_NOT_FOUND) {
		...
		if (!zstr(level)) {
			cc_tier_update("col_level", level, queue, agent);
		} else {
			cc_tier_update("col_level", "1", queue, agent);
		}
		if (!zstr(position)) {
								  ...
								  switch_event_t *params, switch_xml_t x_tiers_cfg)
{
	...
	if (x_tiers_cfg) {
		...
		if (!(x_tiers = switch_xml_child(cfg, "tiers"))) { goto end; }
	}
	...
	for (x_tier = switch_xml_child(x_tiers, "tier"); x_tier; x_tier = x_tier->next) {
		...
		const char *level = switch_xml_attr(x_tier, "col_level");
		const char *position = switch_xml_attr(x_tier, "position");
		if (load_all == SWITCH_TRUE) {
			result = load_tier(queue, agent, level, position);
		} else if (!zstr(agent_name) && !zstr(queue_name) && !strcasecmp(agent, agent_name) &&
				   !strcasecmp(queue, queue_name)) {
			result = load_tier(queue, agent, level, position);
...
static int sqlite_column_rename_callback(void *pArg, const char *errmsg)
{
	...
	if (!strcasecmp("agents", callback_data->table)) {
		if (NULL != (sql = strstr(agents_sql, "TABLE agents ("))) {
			...
			switch_cache_db_execute_sql_callback(callback_data->dbh, "INSERT INTO agents_tmp SELECT * FROM agents;",
												 NULL, NULL, NULL);
			...
			switch_cache_db_execute_sql_callback(callback_data->dbh, "INSERT INTO agents SELECT * FROM agents_tmp;",
												 NULL, NULL, NULL);
		...
		if (NULL != (sql = strstr(members_sql, "TABLE members ("))) {
			...
			switch_cache_db_execute_sql_callback(callback_data->dbh, tmptable, NULL, NULL, NULL);
			switch_cache_db_execute_sql_callback(callback_data->dbh, "INSERT INTO members_tmp SELECT * FROM members;",
												 NULL, NULL, NULL);
			...
			switch_cache_db_execute_sql_callback(callback_data->dbh, "INSERT INTO members SELECT * FROM members_tmp;",
												 NULL, NULL, NULL);
...
static switch_status_t load_config(switch_memory_pool_t *pool)
{
	...
	if (zstr(globals.dbname)) { globals.dbname = strdup(CC_SQLITE_DB_NAME); }
	if (zstr(globals.cc_instance_id)) { globals.cc_instance_id = switch_core_strdup(pool, "single_box"); }
	...
	switch_cache_db_test_reactive(dbh, "select count(ready_time) from agents", NULL,
								  "alter table agents add ready_time integer not null;"
								  "alter table agents add reject_delay_time integer default 0 not null;"
								  "alter table agents add busy_delay_time  integer default 0 not null");
	switch_cache_db_test_reactive(dbh, "select count(no_answer_delay_time) from agents", NULL,
								  "alter table agents add no_answer_delay_time integer default 0 not null;");
	switch_cache_db_test_reactive(dbh, "select count(ready_time) from agents", "drop table agents", agents_sql);
	switch_cache_db_test_reactive(dbh, "select count(external_calls_count) from agents", NULL,
								  "alter table agents add external_calls_count integer default 0 not null;");
	...
	if (switch_cache_db_get_type(dbh) == SCDB_TYPE_CORE_DB) {
		...
		switch_cache_db_execute_sql_callback_err(dbh, "select count(instance_id) from agents", NULL,
												 sqlite_column_rename_callback, &callback_data, &errmsg);
		callback_data.table = "members";
		switch_cache_db_execute_sql_callback_err(dbh, "select count(instance_id) from members", NULL,
												 sqlite_column_rename_callback, &callback_data, &errmsg);
	} else {
		/* This will rename column system for MySQL */
		switch_cache_db_test_reactive(dbh, "select count(instance_id) from agents", NULL,
									  "alter table agents rename column `system` TO instance_id;");
		switch_cache_db_test_reactive(dbh, "select count(instance_id) from members", NULL,
									  "alter table members rename column `system` TO instance_id;");
		/* This will rename column system for PGSql */
		switch_cache_db_test_reactive(dbh, "select count(instance_id) from agents", NULL,
									  "alter table agents rename system TO instance_id;");
		switch_cache_db_test_reactive(dbh, "select count(instance_id) from members", NULL,
									  "alter table members rename system TO instance_id;");
	}
	...
	//					 "update tiers set state = 'Ready' where agent IN (select name from agents where instance_id
	//='single_box');" 						"update members set state = '%q', session_uuid = '' where instance_id = '%q';" 						"update agents set
	//external_calls_count = 0 where instance_id = 'single_box';", 							cc_member_state2str(CC_MEMBER_STATE_ABANDONED),
	...
	sql = switch_mprintf("update members set state = '%q', session_uuid = '' where instance_id = '%q'",
	...
	if ((x_agents = switch_xml_child(cfg, "agents"))) {
		for (x_agent = switch_xml_child(x_agents, "agent"); x_agent; x_agent = x_agent->next) {
			const char *agent = switch_xml_attr(x_agent, "name");
			if (agent) { load_agent(agent, NULL, x_agents); }
		}
	}
	...
	if (str && !strncmp(str, "ARRAY::", 7)) {
		...
		while (1) {
			...
			if (status == SWITCH_STATUS_FALSE /* Invalid Recording */ && SWITCH_READ_ACCEPTABLE(status)) {
				/* Sadly, there doesn't seem to be a return to switch_ivr_play_file that tell you the file wasn't found.
				 * FALSE also mean that the channel got switch to BRAKE state, so we check for read acceptable */
...
static void *SWITCH_THREAD_FUNC outbound_agent_thread_run(switch_thread_t *thread, void *obj)
{
	...
	if (!member_session) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "Member %s <%s> with uuid %s in queue %s is gone just before we assigned an agent\n",
						  h->member_cid_name, h->member_cid_number, h->member_session_uuid, h->queue_name);
		bridged = 0;

		sql =
			switch_mprintf("UPDATE members SET state = '%q', session_uuid = '', abandoned_epoch = '%" SWITCH_TIME_T_FMT
						   "' WHERE uuid = '%q' AND instance_id = '%q' AND state != '%q'",
						   cc_member_state2str(CC_MEMBER_STATE_ABANDONED), local_epoch_time_now(NULL), h->member_uuid,
						   globals.cc_instance_id, cc_member_state2str(CC_MEMBER_STATE_ABANDONED));
	...
	if (!strcasecmp(h->agent_type, CC_AGENT_TYPE_CALLBACK)) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
						  "Setting outbound caller_id_name to: %s\n", cid_name);
		...
		status =
			switch_ivr_originate(NULL, &agent_session, &cause, dialstr, globals.agent_originate_timeout, NULL,
								 cid_name ? cid_name : h->member_cid_name,
								 cid_number ? cid_number : h->member_cid_number, NULL, ovars, SOF_NONE, NULL, NULL);
		...
		if (status == SWITCH_STATUS_SUCCESS) {
			...
			*/
			if (other_loopback_leg_uuid) {
				...
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
								  "Agent '%s' is a loopback channel. Searching for real channel...\n", h->agent_name);

				/* Locate loopback-b channel */
				if (!(loopback_b_session = switch_core_session_locate(other_loopback_leg_uuid))) {
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
									  "Failed to locate loopback-b channel of agent '%s'\n", h->agent_name);
					...
					if (switch_channel_wait_for_flag(loopback_b_channel, CF_BRIDGED, SWITCH_TRUE, 5000,
													 member_channel) != SWITCH_STATUS_SUCCESS) {
						switch_log_printf(
							SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
							"Timeout while waiting for the loopback-b channel to be bridged (agent '%s')\n",
							h->agent_name);
						switch_channel_hangup(loopback_a_channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
						status = SWITCH_STATUS_FALSE;
					} else {
						...
						if (!real_uuid) {
							switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
											  "Failed to find a real channel behind loopback agent '%s'\n",
											  h->agent_name);
							switch_channel_hangup(loopback_a_channel, SWITCH_CAUSE_DESTINATION_OUT_OF_ORDER);
							status = SWITCH_STATUS_FALSE;
						} else {
							...
							if (!(agent_session = switch_core_session_locate(real_uuid))) {
								...
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
												  "Real channel found behind loopback agent '%s'\n", h->agent_name);
								...
								switch_channel_set_variable(agent_channel, "cc_member_session_uuid",
		...
		if (dialstr != h->originate_string) { switch_safe_free(dialstr); }
		...
		if (agent_session) {
			...
			for (int i = 0; i < argc; ++i) {
				switch_channel_set_variable(agent_channel, argv[i],
											switch_channel_get_variable(member_channel, argv[i]));
			}
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
						  "Invalid agent type '%s' for agent '%s', aborting member offering", h->agent_type,
						  h->agent_name);
	...
	if (status == SWITCH_STATUS_SUCCESS) {
		...
		if (!strcasecmp(h->queue_strategy, "ring-all") || !strcasecmp(h->queue_strategy, "ring-progressively")) {
			...
			sql = switch_mprintf("UPDATE members SET serving_agent = '%q', serving_system = 'single_box', state = '%q'"
								 ...
								 cc_member_state2str(CC_MEMBER_STATE_TRYING), h->member_uuid, globals.cc_instance_id,
								 h->queue_strategy);
			cc_execute_sql(NULL, sql, NULL);
			...
			if (atoi(res) == 0) { goto done; }
			switch_core_session_hupall_matching_var("cc_member_pre_answer_uuid", h->member_uuid,
		...
		if (h->record_template) {
			...
			if (expanded != h->record_template) { switch_safe_free(expanded); }
		}

		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
						  "Agent %s answered \"%s\" <%s> from queue %s%s\n", h->agent_name, h->member_cid_name,
						  h->member_cid_number, h->queue_name, (h->record_template ? " (Recorded)" : ""));
		...
		if (switch_true(switch_channel_get_variable(member_channel, SWITCH_BYPASS_MEDIA_AFTER_BRIDGE_VARIABLE)) ||
			switch_true(switch_channel_get_variable(agent_channel, SWITCH_BYPASS_MEDIA_AFTER_BRIDGE_VARIABLE))) {
			switch_channel_set_flag(member_channel, CF_BYPASS_MEDIA_AFTER_BRIDGE);
		}

		if (switch_ivr_uuid_bridge(h->member_session_uuid, switch_core_session_get_uuid(agent_session)) !=
			SWITCH_STATUS_SUCCESS) {
			bridged = 0;
		}

		if (bridged && (switch_channel_wait_for_flag(agent_channel, CF_BRIDGED, SWITCH_TRUE, 3000, NULL) !=
						SWITCH_STATUS_SUCCESS)) {
			bridged = 0;
		}
		...
		if (!bridged && !switch_channel_up(member_channel)) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
							  "Failed to bridge, member \"%s\" <%s> has gone just before we called agent %s\n",
			...
			sql = switch_mprintf(
				"UPDATE members SET state = '%q', session_uuid = '', abandoned_epoch = '%" SWITCH_TIME_T_FMT
				"' WHERE uuid = '%q' AND instance_id = '%q'",
				cc_member_state2str(CC_MEMBER_STATE_ABANDONED), local_epoch_time_now(NULL), h->member_uuid,
				globals.cc_instance_id);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
			...
			sql = switch_mprintf("UPDATE members SET state = 'Waiting' WHERE uuid = '%q' AND instance_id = '%q'",
								 h->member_uuid, globals.cc_instance_id);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		} else {
			bridged = 1;
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
							  "Member \"%s\" %s is bridged to agent %s\n", h->member_cid_name, h->member_cid_number,
							  h->agent_name);
			...
			sql = switch_mprintf("UPDATE members SET state = '%q', bridge_epoch = '%" SWITCH_TIME_T_FMT
								 "' WHERE uuid = '%q' AND instance_id = '%q'",
								 cc_member_state2str(CC_MEMBER_STATE_ANSWERED), local_epoch_time_now(NULL),
								 h->member_uuid, globals.cc_instance_id);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		}
		...
		if (bridged) {
			...
			switch_channel_set_variable_printf(member_channel, "cc_queue_answered_epoch", "%" SWITCH_TIME_T_FMT,
											   local_epoch_time_now(NULL));
			/* Set UUID of the Agent channel */
			sql = switch_mprintf("UPDATE agents SET uuid = '%q', last_bridge_start = '%" SWITCH_TIME_T_FMT
								 "', calls_answered = calls_answered + 1, no_answer_count = 0"
								 " WHERE name = '%q' AND instance_id = '%q'",
								 agent_uuid, local_epoch_time_now(NULL), h->agent_name, h->agent_system);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		...
		/* Wait until the agent hangup.  This will quit also if the agent transfer the call or callcenter_break is
		 * used*/
		while (bridged && switch_channel_up(agent_channel) && globals.running) {
			if (!strcasecmp(h->agent_type, CC_AGENT_TYPE_UUID_STANDBY)) {
				if (!switch_channel_test_flag(agent_channel, CF_BRIDGED)) { break; }
			}
			if (switch_channel_test_app_flag_key(CC_APP_KEY, agent_channel, CC_APP_SHOULD_BREAK)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(agent_session), SWITCH_LOG_DEBUG,
								  "callcenter_break requested at agent channel, releasing agent %s\n", h->agent_name);
		...
		if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CALLCENTER_EVENT) == SWITCH_STATUS_SUCCESS) {
			...
			switch_event_add_header(event, SWITCH_STACK_BOTTOM, "CC-Member-Joined-Time", "%" SWITCH_TIME_T_FMT,
									t_member_called);
			switch_event_add_header(event, SWITCH_STACK_BOTTOM, "CC-Bridge-Terminated-Time", "%" SWITCH_TIME_T_FMT,
									local_epoch_time_now(NULL));
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-UUID", h->member_uuid);
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-Session-UUID",
										   h->member_session_uuid);
		...
		if (bridged) {
			/* for xml_cdr needs */
			switch_channel_set_variable_printf(member_channel, "cc_queue_terminated_epoch", "%" SWITCH_TIME_T_FMT,
											   local_epoch_time_now(NULL));

			/* Update Agents Items */
			/* Do not remove uuid of the agent if we are a standby agent */
			sql =
				switch_mprintf("UPDATE agents SET %s last_bridge_end = %" SWITCH_TIME_T_FMT
							   ", talk_time = talk_time + (%" SWITCH_TIME_T_FMT
							   "-last_bridge_start) WHERE name = '%q' AND instance_id = '%q';",
							   (strcasecmp(h->agent_type, CC_AGENT_TYPE_UUID_STANDBY) ? "uuid = ''," : ""),
							   local_epoch_time_now(NULL), local_epoch_time_now(NULL), h->agent_name, h->agent_system);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);

			/* Remove the member entry from the db (Could become optional to support latter processing) */
			sql = switch_mprintf("DELETE FROM members WHERE uuid = '%q' AND instance_id = '%q'", h->member_uuid,
								 globals.cc_instance_id);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		...
		switch_channel_clear_app_flag_key(CC_APP_KEY, member_channel, CC_APP_AGENT_CONNECTING);
		sql = switch_mprintf(
			"UPDATE members SET state = case state when '%q' then '%q' else state end, serving_agent = '', "
			"serving_system = ''"
			" WHERE serving_agent = '%q' AND serving_system = '%q' AND uuid = '%q' AND instance_id = '%q'",
			cc_member_state2str(CC_MEMBER_STATE_TRYING), /* Only switch to Waiting from Trying (state may be set to
															Abandoned in callcenter_function()) */
		...
		case SWITCH_CAUSE_USER_BUSY:
			delay_next_agent_call =
				(h->busy_delay_time > delay_next_agent_call ? h->busy_delay_time : delay_next_agent_call);
			break;
		/* Reject: User rejected the call */
		case SWITCH_CAUSE_CALL_REJECTED:
			delay_next_agent_call =
				(h->reject_delay_time > delay_next_agent_call ? h->reject_delay_time : delay_next_agent_call);
			break;
		...
		default:
			delay_next_agent_call =
				(h->no_answer_delay_time > delay_next_agent_call ? h->no_answer_delay_time : delay_next_agent_call);

			tiers_state = CC_TIER_STATE_NO_ANSWER;

			/* Update Agent NO Answer count */
			sql = switch_mprintf(
				"UPDATE agents SET no_answer_count = no_answer_count + 1 WHERE name = '%q' AND instance_id = '%q';",
			...
			if (h->max_no_answer > 0 && (h->no_answer_count + 1) >= h->max_no_answer) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
								  ...
								  h->max_no_answer, cc_agent_status2str(h->agent_no_answer_status));

				if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CALLCENTER_EVENT) ==
					SWITCH_STATUS_SUCCESS) {
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Agent", h->agent_name);
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Action", "agent-max-no-answer");
					switch_event_add_header(event, SWITCH_STACK_BOTTOM, "CC-Agent-No-Answer-Count", "%d",
											h->max_no_answer);
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Agent-No-Answer-Status",
												   cc_agent_status2str(h->agent_no_answer_status));
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-UUID", h->member_uuid);
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-Session-UUID",
												   h->member_session_uuid);
					switch_event_fire(&event);
		...
		if (delay_next_agent_call > 0) {
			char ready_epoch[64];
			switch_snprintf(ready_epoch, sizeof(ready_epoch), "%" SWITCH_TIME_T_FMT,
							local_epoch_time_now(NULL) + delay_next_agent_call);
		...
		if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CALLCENTER_EVENT) == SWITCH_STATUS_SUCCESS) {
			...
			switch_event_add_header(event, SWITCH_STACK_BOTTOM, "CC-Agent-Called-Time", "%" SWITCH_TIME_T_FMT,
									t_agent_called);
			switch_event_add_header(event, SWITCH_STACK_BOTTOM, "CC-Agent-Aborted-Time", "%" SWITCH_TIME_T_FMT,
									local_epoch_time_now(NULL));
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-UUID", h->member_uuid);
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-Session-UUID",
										   h->member_session_uuid);
	...
	sql = switch_mprintf("UPDATE tiers SET state = '%q' WHERE agent = '%q' AND queue = '%q' AND (state = '%q' OR state "
						 "= '%q' OR state = '%q')",
						 cc_tier_state2str(tiers_state), h->agent_name, h->queue_name,
						 cc_tier_state2str(CC_TIER_STATE_ACTIVE_INBOUND), cc_tier_state2str(CC_TIER_STATE_STANDBY),
						 cc_tier_state2str(CC_TIER_STATE_OFFERING));
	cc_execute_sql(NULL, sql, NULL);
	switch_safe_free(sql);

	sql = switch_mprintf("UPDATE tiers SET state = '%q' WHERE agent = '%q' AND NOT queue = '%q' AND state = '%q'",
						 cc_tier_state2str(CC_TIER_STATE_READY), h->agent_name, h->queue_name,
						 cc_tier_state2str(CC_TIER_STATE_STANDBY));
	cc_execute_sql(NULL, sql, NULL);
	switch_safe_free(sql);

	/* If we are in Status Available On Demand, set state to Idle so we do not receive another call until state manually
	 * changed to Waiting */
	...
	if (agent_session) { switch_core_session_rwunlock(agent_session); }
	if (member_session) { switch_core_session_rwunlock(member_session); }
...
static int agents_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	if (cbt->tier_rules_apply == SWITCH_TRUE && atoi(agent_tier_level) > cbt->tier) {
		...
		} else if (cbt->tier_rule_wait_multiply_level == SWITCH_TRUE &&
				   (long)local_epoch_time_now(NULL) - atol(cbt->member_joined_epoch) >=
					   atoi(agent_tier_level) * (int)cbt->tier_rule_wait_second) {
			cbt->tier = atoi(agent_tier_level);
			cbt->tier_agent_available = 0;
			/* Just check if joined is bigger than next tier wait time */
		} else if (cbt->tier_rule_wait_multiply_level == SWITCH_FALSE &&
				   (long)local_epoch_time_now(NULL) - atol(cbt->member_joined_epoch) >=
					   (int)cbt->tier_rule_wait_second) {
			cbt->tier = atoi(agent_tier_level);
			cbt->tier_agent_available = 0;
		} else {
	...
	if (!(!strcasecmp(agent_state, cc_agent_state2str(CC_AGENT_STATE_WAITING)))) { contact_agent = SWITCH_FALSE; }
	if (!(atol(agent_last_bridge_end) < ((long)local_epoch_time_now(NULL) - atol(agent_wrap_up_time)))) {
		contact_agent = SWITCH_FALSE;
	}
	if (!(atol(agent_ready_time) <= (long)local_epoch_time_now(NULL))) { contact_agent = SWITCH_FALSE; }
	if (!(strcasecmp(agent_status, cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK)))) { contact_agent = SWITCH_FALSE; }
	/* XXX callcenter_track app can update this counter after we selected this agent on database */
	if (cbt->skip_agents_with_external_calls && atoi(agent_external_calls_count) > 0) { contact_agent = SWITCH_FALSE; }
	if (contact_agent == SWITCH_FALSE) { return 0; /* Continue to next Agent */ }
	...
	if (globals.reserve_agents) {
		...
		if (cc_agent_update("state_if_waiting", cc_agent_state2str(CC_AGENT_STATE_RESERVED), agent_name) ==
			CC_STATUS_SUCCESS) {
	...
	if (!strcasecmp(cbt->strategy, "ring-all") || !strcasecmp(cbt->strategy, "ring-progressively")) {
		...
		sql = switch_mprintf(
			"SELECT count(*) FROM members WHERE serving_agent = '%q' AND uuid = '%q' AND instance_id = '%q'",
			cbt->strategy, cbt->member_uuid, globals.cc_instance_id);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		...
		/* Check if we won the race to get the member to our selected agent (Used for Multi system purposes) */
		sql = switch_mprintf("SELECT count(*) FROM members WHERE serving_agent = '%q' AND serving_system = '%q' AND "
							 "uuid = '%q' AND instance_id = '%q'",
	...
	default: /* Go ahead, start thread to try to bridge these 2 caller */
	{
		...
		if (!strcasecmp(cbt->strategy, "ring-progressively")) {
			...
			if (member_session) {
				switch_channel_t *member_channel = switch_core_session_get_channel(member_session);
				switch_channel_set_variable_printf(member_channel, "cc_last_originated_call", "%" SWITCH_TIME_T_FMT,
												   local_epoch_time_now(NULL));
		...
		switch_safe_free(sql);
		sql = switch_mprintf("UPDATE tiers SET state = '%q' WHERE agent = '%q' AND NOT queue = '%q' AND state = '%q'",
							 cc_tier_state2str(CC_TIER_STATE_STANDBY), h->agent_name, h->queue_name,
							 cc_tier_state2str(CC_TIER_STATE_READY));
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
...
static int members_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	if (!cbt.queue_name || !(queue = get_queue(cbt.queue_name))) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Queue %s not found locally, delete this member\n",
						  cbt.queue_name);
		sql = switch_mprintf("DELETE FROM members WHERE uuid = '%q' AND instance_id = '%q'", cbt.member_uuid,
							 cbt.member_system);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
		goto end;
		...
		if (queue->ring_progressively_delay) { ring_progressively_delay = queue->ring_progressively_delay; }

		if (queue->record_template) { queue_record_template = strdup(queue->record_template); }
	...
	if (!strcasecmp(member_state, cc_member_state2str(CC_MEMBER_STATE_ABANDONED))) {
		...
		if (abandoned_epoch == 0) { abandoned_epoch = atoll(cbt.member_joined_epoch); }
		...
		if (abandoned_epoch + discard_abandoned_after < local_epoch_time_now(NULL)) {
			sql = switch_mprintf("DELETE FROM members WHERE uuid = '%q' AND instance_id = '%q' AND (abandoned_epoch = "
								 "'%" SWITCH_TIME_T_FMT "' OR joined_epoch = '%q')",
								 cbt.member_uuid, cbt.member_system, abandoned_epoch, cbt.member_joined_epoch);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
		}
		/* Skip this member */
	...
	if (!strcasecmp(serving_agent, "ring-all") && (strcasecmp(queue_strategy, "ring-all") != 0)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "Queue '%s' changed strategy, adjusting member parameters", queue_name);
		...
		if (!strcasecmp(queue_strategy, "ring-progressively")) {
			sql = switch_mprintf("UPDATE members SET serving_agent = 'ring-progressively' WHERE uuid = '%q' AND state "
								 "= '%q' AND serving_agent = 'ring-all'",
								 cbt.member_uuid, cc_member_state2str(CC_MEMBER_STATE_TRYING));
		} else {
			sql = switch_mprintf("UPDATE members SET serving_agent = '', state = '%q' WHERE uuid = '%q' AND state = "
								 "'%q' AND serving_agent = 'ring-all'",
	...
	/* member is ring-progressively but not the queue */
	else if (!strcasecmp(serving_agent, "ring-progressively") &&
			 (strcasecmp(queue_strategy, "ring-progressively") != 0)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "Queue '%s' changed strategy, adjusting member parameters", queue_name);
		...
		if (!strcasecmp(queue_strategy, "ring-all")) {
			sql = switch_mprintf("UPDATE members SET serving_agent = 'ring-all' WHERE uuid = '%q' AND state = '%q' AND "
								 "serving_agent = 'ring-progressively'",
								 cbt.member_uuid, cc_member_state2str(CC_MEMBER_STATE_TRYING));
		} else {
			sql = switch_mprintf("UPDATE members SET serving_agent = '', state = '%q' WHERE uuid = '%q' AND state = "
								 "'%q' AND serving_agent = 'ring-progressively'",
								 cc_member_state2str(CC_MEMBER_STATE_WAITING), cbt.member_uuid,
								 cc_member_state2str(CC_MEMBER_STATE_TRYING));
		}
		cc_execute_sql(NULL, sql, NULL);
	...
	/* Queue is now ring-all and not the member */
		...
		if (!strcasecmp(serving_agent, "ring-progressively")) {
			...
			sql = switch_mprintf("UPDATE members SET serving_agent = 'ring-all', state = '%q' WHERE uuid = '%q' AND "
								 "state = '%q' AND serving_agent = ''",
								 cc_member_state2str(CC_MEMBER_STATE_TRYING), cbt.member_uuid,
								 cc_member_state2str(CC_MEMBER_STATE_WAITING));
		}
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
	...
	/* Queue is now ring-progressively and not the member */
	else if (!strcasecmp(queue_strategy, "ring-progressively") &&
			 (strcasecmp(serving_agent, "ring-progressively") != 0)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
						  "Queue '%s' changed strategy, adjusting member parameters", queue_name);
		...
		if (!strcasecmp(serving_agent, "ring-all")) {
			...
			sql = switch_mprintf("UPDATE members SET serving_agent = 'ring-progressively', state = '%q' WHERE uuid = "
								 "'%q' AND state = '%q' AND serving_agent = ''",
								 cc_member_state2str(CC_MEMBER_STATE_TRYING), cbt.member_uuid,
								 cc_member_state2str(CC_MEMBER_STATE_WAITING));
		}
		cc_execute_sql(NULL, sql, NULL);
	...
	if (zstr(cbt.member_session_uuid)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING,
						  "Member %s <%s> in Queue %s have no session uuid, skip this member\n", cbt.member_cid_name,
						  cbt.member_cid_number, cbt.queue_name);
	}
	...
	if (!strcasecmp(queue->strategy, "top-down")) {
		/* WARNING this use channel variable to help dispatch... might need to be reviewed to save it in DB to make this
		 * multi server prooft in the future */
		switch_core_session_t *member_session = switch_core_session_locate(cbt.member_session_uuid);
		...
		if (member_session) {
			...
			if ((last_agent_tier_position =
					 switch_channel_get_variable(member_channel, "cc_last_agent_tier_position"))) {
		...
		sql = switch_mprintf(
			...
			"busy_delay_time, no_answer_delay_time, tiers.state, agents.last_bridge_end, agents.wrap_up_time, "
			"agents.state, agents.ready_time, tiers.position as tiers_position, tiers.col_level as tiers_level, "
			"agents.type, agents.uuid, external_calls_count, agents.last_offered_call as agents_last_offered_call, 1 "
			"as dyn_order FROM agents LEFT JOIN tiers ON (agents.name = tiers.agent)"
			" WHERE tiers.queue = '%q'"
			" AND (agents.status = '%q' OR agents.status = '%q' OR agents.status = '%q')"
			" AND tiers.position > %d"
			" AND tiers.col_level = %d"
			" UNION "
			"SELECT instance_id, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, "
			"busy_delay_time, no_answer_delay_time, tiers.state, agents.last_bridge_end, agents.wrap_up_time, "
			"agents.state, agents.ready_time, tiers.position as tiers_position, tiers.col_level as tiers_level, "
			"agents.type, agents.uuid, external_calls_count, agents.last_offered_call as agents_last_offered_call, 2 "
			"as dyn_order FROM agents LEFT JOIN tiers ON (agents.name = tiers.agent)"
			" WHERE tiers.queue = '%q'"
			" AND (agents.status = '%q' OR agents.status = '%q' OR agents.status = '%q')"
			" AND tiers.col_level > %d"
			" ORDER BY dyn_order asc, tiers_level, tiers_position, agents_last_offered_call",
			queue_name, cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE), cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK),
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE_ON_DEMAND), position, level, queue_name,
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE), cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK),
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE_ON_DEMAND), level);
		...
		sql = switch_mprintf(
			...
			" WHERE tiers.queue = '%q'"
			" AND (agents.status = '%q' OR agents.status = '%q' OR agents.status = '%q')"
			" AND tiers.position > (SELECT tiers.position FROM agents LEFT JOIN tiers ON (agents.name = tiers.agent) "
			"WHERE tiers.queue = '%q' AND agents.last_offered_call > 0 ORDER BY agents.last_offered_call DESC LIMIT 1)"
			" AND tiers.col_level = (SELECT tiers.col_level FROM agents LEFT JOIN tiers ON (agents.name = tiers.agent) "
			"WHERE tiers.queue = '%q' AND agents.last_offered_call > 0 ORDER BY agents.last_offered_call DESC LIMIT 1)"
			" UNION "
			"SELECT instance_id, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, "
			"busy_delay_time, no_answer_delay_time, tiers.state, agents.last_bridge_end, agents.wrap_up_time, "
			"agents.state, agents.ready_time, tiers.position as tiers_position, tiers.level as tiers_level, "
			"agents.type, agents.uuid, external_calls_count, agents.last_offered_call as agents_last_offered_call, 2 "
			"as dyn_order FROM agents LEFT JOIN tiers ON (agents.name = tiers.agent)"
			" WHERE tiers.queue = '%q'"
			" AND (agents.status = '%q' OR agents.status = '%q' OR agents.status = '%q')"
			" ORDER BY dyn_order asc, tiers_level, tiers_position, agents_last_offered_call",
			queue_name, cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE), cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK),
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE_ON_DEMAND), queue_name, queue_name, queue_name,
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE), cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK),
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE_ON_DEMAND));
		...
		if (!strcasecmp(queue->strategy, "longest-idle-agent")) {
			sql_order_by = switch_mprintf("col_level, agents.last_bridge_end, position");
		} else if (!strcasecmp(queue_strategy, "agent-with-least-talk-time")) {
			sql_order_by = switch_mprintf("col_level, agents.talk_time, position");
		} else if (!strcasecmp(queue_strategy, "agent-with-fewest-calls")) {
			sql_order_by = switch_mprintf("col_level, agents.calls_answered, position");
		} else if (!strcasecmp(queue_strategy, "ring-all") || !strcasecmp(queue_strategy, "ring-progressively")) {
			sql = switch_mprintf(
				"UPDATE members SET state = '%q' WHERE state = '%q' AND uuid = '%q' AND instance_id = '%q'",
				cc_member_state2str(CC_MEMBER_STATE_TRYING), cc_member_state2str(CC_MEMBER_STATE_WAITING),
				cbt.member_uuid, cbt.member_system);
			cc_execute_sql(NULL, sql, NULL);
			switch_safe_free(sql);
			sql_order_by = switch_mprintf("col_level, position");
		} else if (!strcasecmp(queue_strategy, "random")) {
			sql_order_by = switch_mprintf("col_level, random()");
		} else if (!strcasecmp(queue_strategy, "sequentially-by-agent-order")) {
			sql_order_by = switch_mprintf(
				"col_level, position, agents.last_offered_call"); /* Default to last_offered_call, let add new strategy
																	 if needing it differently */
		} else {
			/* If the strategy doesn't exist, just fallback to the following */
			sql_order_by = switch_mprintf("col_level, position, agents.last_offered_call");
		}

		sql = switch_mprintf(
			"SELECT instance_id, name, status, contact, no_answer_count, max_no_answer, reject_delay_time, "
			"busy_delay_time, no_answer_delay_time, tiers.state, agents.last_bridge_end, agents.wrap_up_time, "
			"agents.state, agents.ready_time, tiers.position, tiers.col_level, agents.type, agents.uuid, "
			"external_calls_count FROM agents LEFT JOIN tiers ON (agents.name = tiers.agent)"
			" WHERE tiers.queue = '%q'"
			" AND (agents.status = '%q' OR agents.status = '%q' OR agents.status = '%q')"
			" ORDER BY %q",
			queue_name, cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE), cc_agent_status2str(CC_AGENT_STATUS_ON_BREAK),
			cc_agent_status2str(CC_AGENT_STATUS_AVAILABLE_ON_DEMAND), sql_order_by);
	...
	if (!strcasecmp(queue->strategy, "ring-progressively")) {
		...
		if (member_session) {
			...
			if (last_originated_call && switch_channel_ready(member_channel) &&
				((long)local_epoch_time_now(NULL) < atoi(last_originated_call) + ring_progressively_delay) &&
				!switch_true(switch_channel_get_variable(member_channel, "cc_agent_found"))) {
...
void *SWITCH_THREAD_FUNC cc_agent_dispatch_thread_run(switch_thread_t *thread, void *obj)
{
	...
	while (globals.running == 1) {
		char *sql = NULL;
		sql = switch_mprintf(
			"SELECT queue,uuid,session_uuid,cid_number,cid_name,joined_epoch,(%" SWITCH_TIME_T_FMT
			"-joined_epoch)+base_score+skill_score AS score, state, abandoned_epoch, serving_agent, instance_id FROM "
			"members"
			" WHERE (state = '%q' OR state = '%q' OR (serving_agent = 'ring-all' AND state = '%q') OR (serving_agent = "
			"'ring-progressively' AND state = '%q')) AND instance_id = '%q' ORDER BY score DESC",
			local_epoch_time_now(NULL), cc_member_state2str(CC_MEMBER_STATE_WAITING),
			cc_member_state2str(CC_MEMBER_STATE_ABANDONED), cc_member_state2str(CC_MEMBER_STATE_TRYING),
			cc_member_state2str(CC_MEMBER_STATE_TRYING), globals.cc_instance_id);

		cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, members_callback,
								NULL /* Call back variables */);
		switch_safe_free(sql);
		switch_yield(100000);
	}
	...
	while (switch_channel_ready(member_channel) && m->running && globals.running) {
		...
		if (queue->max_wait_time > 0 && queue->max_wait_time <= time_now - m->t_member_called) {
			...
			if (switch_channel_test_app_flag_key(CC_APP_KEY, member_channel, CC_APP_AGENT_CONNECTING)) {
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
								  "Member %s <%s> in queue '%s' reached max wait time and we're connecting, waiting "
								  "for agent to be connected...\n",
		...
		if (queue->max_wait_time_with_no_agent > 0 && queue->last_agent_exist_check > queue->last_agent_exist &&
			...
			if (queue->last_agent_exist_check - queue->last_agent_exist >= queue->max_wait_time_with_no_agent) {
				...
				if (queue->max_wait_time_with_no_agent_time_reached > 0) {
					...
					if (queue->last_agent_exist_check - m->t_member_called >=
						queue->max_wait_time_with_no_agent_time_reached + queue->max_wait_time_with_no_agent) {
						switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
										  "Member %s <%s> in queue '%s' reached max wait of %d sec. with no agent plus "
										  "join grace period of %d sec.\n",
										  m->member_cid_name, m->member_cid_number, m->queue_name,
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
									  "Member %s <%s> in queue '%s' reached max wait of %d sec. with no agent\n",
									  m->member_cid_name, m->member_cid_number, m->queue_name,
									  queue->max_wait_time_with_no_agent);
					m->member_cancel_reason = CC_MEMBER_CANCEL_REASON_NO_AGENT_TIMEOUT;
					switch_channel_set_flag_value(member_channel, CF_BREAK, 2);
		...
		/* SAMPLE CODE to playback something over the MOH
		   ...
		   switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "execute-app-arg",
		   "tone_stream://%(200,0,500,600,700)"); switch_core_session_queue_private_event(member_session, &event,
		   SWITCH_TRUE);
		   }
		 */
...
static switch_status_t moh_on_dtmf(switch_core_session_t *session, void *input, switch_input_type_t itype, void *buf,
								   unsigned int buflen)
{
...
SWITCH_STANDARD_APP(callcenter_function)
{
	...
	if (queue->abandoned_resume_allowed == SWITCH_TRUE) {
		...
		/* Check to see if agent already exist */
		sql =
			switch_mprintf("SELECT uuid FROM members WHERE queue = '%q' AND cid_number = '%q' AND state = '%q' ORDER "
						   "BY abandoned_epoch DESC",
						   queue_name, switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")),
	...
	if (cc_base_score) { cc_base_score_int += atoi(cc_base_score); }
	...
	/* for xml_cdr needs */
	switch_channel_set_variable_printf(member_channel, "cc_queue_joined_epoch", "%" SWITCH_TIME_T_FMT,
									   local_epoch_time_now(NULL));
	...
	if (abandoned_epoch > 0) {
		...
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
						  ...
						  switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")), queue_name);

		/* Update abandoned member */
		sql = switch_mprintf(
			"UPDATE members SET session_uuid = '%q', state = '%q', rejoined_epoch = '%" SWITCH_TIME_T_FMT
			"', instance_id = '%q' WHERE uuid = '%q' AND state = '%q'",
			member_session_uuid, cc_member_state2str(CC_MEMBER_STATE_WAITING), local_epoch_time_now(NULL),
			globals.cc_instance_id, member_uuid, cc_member_state2str(CC_MEMBER_STATE_ABANDONED));
		cc_execute_sql(queue, sql, NULL);
		switch_safe_free(sql);

		/* Confirm we took that member in */
		sql =
			switch_mprintf("SELECT abandoned_epoch FROM members WHERE uuid = '%q' AND session_uuid = '%q' AND state = "
						   "'%q' AND queue = '%q'",
						   member_uuid, member_session_uuid, cc_member_state2str(CC_MEMBER_STATE_WAITING), queue_name);
		cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
		switch_safe_free(sql);
		abandoned_epoch = atol(res);
		...
		if (abandoned_epoch == 0) {
			...
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_ERROR,
							  "Member %s <%s> restoring action failed in queue %s, joining again\n",
							  switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_name")),
	...
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CALLCENTER_EVENT) == SWITCH_STATUS_SUCCESS) {
		...
		switch_event_add_header(event, SWITCH_STACK_BOTTOM, "CC-Action", "member-queue-%s",
								(abandoned_epoch == 0 ? "start" : "resume"));
		...
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-CID-Name",
									   switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_name")));
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-CID-Number",
									   switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")));
		switch_event_fire(&event);
	...
	if (abandoned_epoch == 0) {
		...
		sql = switch_mprintf("INSERT INTO members"
							 " (queue,instance_id,uuid,session_uuid,system_epoch,joined_epoch,base_score,skill_score,"
							 "cid_number,cid_name,serving_agent,serving_system,state)"
							 " VALUES('%q','%q','%q','%q','%q','%" SWITCH_TIME_T_FMT
							 "','%d','%d','%q','%q','%q','','%q')",
							 queue_name, globals.cc_instance_id, member_uuid, member_session_uuid, start_epoch,
							 local_epoch_time_now(NULL), cc_base_score_int, 0 /*TODO SKILL score*/,
							 switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")),
							 switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_name")),
							 strategy_str, cc_member_state2str(CC_MEMBER_STATE_WAITING));
		cc_execute_sql(queue, sql, NULL);
		switch_safe_free(sql);
	}
	...
	h->member_cid_name =
		switch_core_strdup(h->pool, switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_name")));
	h->member_cid_number =
		switch_core_strdup(h->pool, switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")));
	...
	while (switch_channel_ready(member_channel)) {
		...
		if (h->member_cancel_reason != CC_MEMBER_CANCEL_REASON_NONE) { break; }
		...
		if (moh_valid && moh_expanded) {
			...
			switch_bool_t exiting_with_key =
				ht.exit_keys && *(ht.exit_keys) && !zstr(&ht.dtmf) && strchr(ht.exit_keys, ht.dtmf);
			if (status == SWITCH_STATUS_FALSE /* Invalid Recording */ && SWITCH_READ_ACCEPTABLE(status)) {
				/* Sadly, there doesn't seem to be a return to switch_ivr_play_file that tell you the file wasn't found.
				 * FALSE also mean that the channel got switch to BRAKE state, so we check for read acceptable */
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_WARNING,
								  "Couldn't play file '%s', continuing wait with no audio\n", cur_moh);
	...
	if (moh_expanded != cur_moh) { switch_safe_free(moh_expanded); }
	...
	if (!switch_channel_up(member_channel) || h->member_cancel_reason != CC_MEMBER_CANCEL_REASON_NONE) {
		...
		switch_core_session_hupall_matching_var("cc_member_pre_answer_uuid", member_uuid,
												SWITCH_CAUSE_ORIGINATOR_CANCEL);
		...
		if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CALLCENTER_EVENT) == SWITCH_STATUS_SUCCESS) {
			...
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Cancel-Reason",
										   cc_member_cancel_reason2str(h->member_cancel_reason));
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-UUID", member_uuid);
			switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "CC-Member-Session-UUID", member_session_uuid);
			switch_event_add_header_string(
				event, SWITCH_STACK_BOTTOM, "CC-Member-CID-Name",
				switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_name")));
			switch_event_add_header_string(
				event, SWITCH_STACK_BOTTOM, "CC-Member-CID-Number",
				switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")));
			switch_event_fire(&event);
		}

		/* Update some channel variables for xml_cdr needs */
		switch_channel_set_variable_printf(member_channel, "cc_queue_canceled_epoch", "%" SWITCH_TIME_T_FMT,
										   local_epoch_time_now(NULL));
		switch_channel_set_variable_printf(member_channel, "cc_cause", "%s", "cancel");
		switch_channel_set_variable_printf(member_channel, "cc_cancel_reason", "%s",
										   cc_member_cancel_reason2str(h->member_cancel_reason));

		/* Print some debug log information */
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(member_session), SWITCH_LOG_DEBUG,
						  "Member \"%s\" <%s> exit queue %s due to %s\n",
						  switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_name")),
						  switch_str_nil(switch_channel_get_variable(member_channel, "caller_id_number")), queue_name,
						  cc_member_cancel_reason2str(h->member_cancel_reason));
		if ((queue = get_queue(queue_name))) {
...
static switch_status_t cc_hook_state_run(switch_core_session_t *session)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
					  "Called cc_hook_hanguphook channel %s with state %s", switch_channel_get_name(channel),
					  switch_channel_state_name(state));

	if (state == CS_HANGUP) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
						  "Tracked call for agent %s ended, decreasing external_calls_count", agent_name);
		sql = switch_mprintf("UPDATE agents SET external_calls_count = external_calls_count - 1 WHERE name = '%q'",
							 agent_name);
		cc_execute_sql(NULL, sql, NULL);
		switch_safe_free(sql);
...
static void cc_send_presence(const char *queue_name)
{
	...
	sql = switch_mprintf("SELECT COUNT(*) FROM members WHERE queue = '%q' AND state = '%q'", queue_name,
						 cc_member_state2str(CC_MEMBER_STATE_WAITING));
	cc_execute_sql2str(NULL, NULL, sql, res, sizeof(res));
	count = atoi(res);
	...
	if (switch_event_create(&send_event, SWITCH_EVENT_PRESENCE_IN) == SWITCH_STATUS_SUCCESS) {
		...
		switch_event_add_header_string(send_event, SWITCH_STACK_BOTTOM, "channel-state",
									   count > 0 ? "CS_ROUTING" : "CS_HANGUP");
		switch_event_add_header_string(send_event, SWITCH_STACK_BOTTOM, "unique-id", queue_name);
		switch_event_add_header_string(send_event, SWITCH_STACK_BOTTOM, "answer-state",
									   count > 0 ? "confirmed" : "terminated");
...
static void cc_presence_event_handler(switch_event_t *event)
{
	...
	//	DUMP_EVENT(event);
	if (!to || strncasecmp(to, "callcenter+", 11) || !strchr(to, '@')) { return; }
...
static int list_result_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	if (cbt->row_process == 1) {
		for (i = 0; i < argc; i++) {
			...
			if (i < argc - 1) { cbt->stream->write_function(cbt->stream, "|"); }
		}
...
static int list_result_json_callback(void *pArg, int argc, char **argv, char **columnNames)
{
	...
	cbt->row_process++;
	for (i = 0; i < argc; i++) { cJSON_AddItemToObject(o, columnNames[i], cJSON_CreateString(argv[i])); }
	cJSON_AddItemToArray(cbt->json_reply, o);
	return 0;
}

#define CC_CONFIG_API_SYNTAX                                                                                           \
...
SWITCH_STANDARD_API(cc_config_api_function)
{
	...
	int argc;
	if (!globals.running) { return SWITCH_STATUS_FALSE; }
	if (zstr(cmd)) {
	...
	if (section && !strcasecmp(section, "agent")) {
		if (action && !strcasecmp(action, "add")) {
			...
			cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_callback,
									&cbt /* Call back variables */);
			switch_safe_free(sql);
			stream->write_function(stream, "%s", "+OK\n");
		}
		...
		if (action && !strcasecmp(action, "add")) {
			if (argc - initial_argc < 2) {
				...
				if (!zstr(level)) { i_level = atoi(level); }
				if (!zstr(position)) { i_position = atoi(position); }
			...
			if (argc - initial_argc < 1) {
				...
				if (!strcasecmp(queue, "all")) { load_all = SWITCH_TRUE; }
			...
			cbt.row_process = 0;
			cbt.stream = stream;
			sql = switch_mprintf("SELECT * FROM tiers ORDER BY col_level, position");
			cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_callback,
									&cbt /* Call back variables */);
			switch_safe_free(sql);
			stream->write_function(stream, "%s", "+OK\n");
		...
		if (action && !strcasecmp(action, "load")) {
			...
			if (argc - initial_argc < 1) {
				...
				stream->write_function(
					...
					"max_wait_time_with_no_agent_time_reached|record_template|calls_answered|calls_abandoned|ring_"
					"progressively_delay|skip_agents_with_external_calls|agent_no_answer_status\n");
				switch_mutex_lock(globals.mutex);
				for (hi = switch_core_hash_first(globals.queue_hash); hi; hi = switch_core_hash_next(&hi)) {
					...
					stream->write_function(
						stream, "%s|%s|%s|%s|%s|%d|%s|%s|%d|%s|%d|%d|%d|%s|%d|%d|%d|%s|%s\n", queue->name,
						queue->strategy, queue->moh, queue->time_base_score,
						(queue->tier_rules_apply ? "true" : "false"), queue->tier_rule_wait_second,
						(queue->tier_rule_wait_multiply_level ? "true" : "false"),
						(queue->tier_rule_no_agent_no_wait ? "true" : "false"), queue->discard_abandoned_after,
						(queue->abandoned_resume_allowed ? "true" : "false"), queue->max_wait_time,
						queue->max_wait_time_with_no_agent, queue->max_wait_time_with_no_agent_time_reached,
						queue->record_template, queue->calls_answered, queue->calls_abandoned,
						queue->ring_progressively_delay, (queue->skip_agents_with_external_calls ? "true" : "false"),
				...
				if (sub_action && !strcasecmp(sub_action, "agents")) {
					if (argc - initial_argc > 2) { status = argv[2 + initial_argc]; }
					if (argc - initial_argc > 3) { state = argv[3 + initial_argc]; }
					if (state) {
						sql = switch_mprintf("SELECT agents.* FROM agents,tiers WHERE tiers.agent = agents.name AND "
											 "tiers.queue = '%q' AND agents.status = '%q' AND agents.state = '%q'",
											 queue_name, status, state);
					} else if (status) {
						sql = switch_mprintf("SELECT agents.* FROM agents,tiers WHERE tiers.agent = agents.name AND "
											 "tiers.queue = '%q' AND agents.status = '%q'",
											 queue_name, status);
					} else {
						sql = switch_mprintf(
							"SELECT agents.* FROM agents,tiers WHERE tiers.agent = agents.name AND tiers.queue = '%q'",
							queue_name);
					}
					/* queue list members */
				} else if (sub_action && !strcasecmp(sub_action, "members")) {
					if (queue_name) {
						sql = switch_mprintf("SELECT  *,(%" SWITCH_TIME_T_FMT
											 "-joined_epoch)+base_score+skill_score AS score FROM members WHERE queue "
											 "= '%q' ORDER BY score DESC;",
											 local_epoch_time_now(NULL), queue_name);
					} else {
						sql = switch_mprintf(
							"SELECT  *,(%" SWITCH_TIME_T_FMT
							"-joined_epoch)+base_score+skill_score AS score FROM members ORDER BY queue, score DESC;",
							local_epoch_time_now(NULL));
					}
					/* queue list tiers */
			...
			if (argc - initial_argc < 1) {
				...
				if (sub_action && !strcasecmp(sub_action, "agents")) {
					...
					if (argc - initial_argc > 3) { state = argv[3 + initial_argc]; }
					if (state) {
						sql = switch_mprintf("SELECT count(*) FROM agents,tiers WHERE tiers.agent = agents.name AND "
											 "tiers.queue = '%q' AND agents.status = '%q' AND agents.state = '%q'",
											 queue_name, status, state);
					} else if (status) {
						sql = switch_mprintf("SELECT count(*) FROM agents,tiers WHERE tiers.agent = agents.name AND "
						...
						sql = switch_mprintf(
							"SELECT count(*) FROM agents,tiers WHERE tiers.agent = agents.name AND tiers.queue = '%q'",
							queue_name);
					}
					/* queue count members */
				} else if (sub_action && !strcasecmp(sub_action, "members")) {
...
SWITCH_STANDARD_API(cc_break_api_function)
{
	...
	if (break_session) { switch_core_session_rwunlock(break_session); }
...
SWITCH_STANDARD_JSON_API(json_callcenter_config_function)
{
	...
	if (zstr(arguments)) { return SWITCH_STATUS_FALSE; }
	...
	if (!strcasecmp(arguments, "agent list")) {
		...
		sql = switch_mprintf("SELECT * FROM agents");
		cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_json_callback,
								&cbt /* Call back variables */);
		switch_safe_free(sql);
	...
	if (!strcasecmp(arguments, "queue list")) {
		...
		for (hi = switch_core_hash_first(globals.queue_hash); hi; hi = switch_core_hash_next(&hi)) {
			...
			cJSON_AddItemToObject(o, "tier_rules_apply",
								  cJSON_CreateString(queue->tier_rules_apply ? "true" : "false"));
			cJSON_AddItemToObject(o, "tier_rule_wait_second", cJSON_CreateNumber(queue->tier_rule_wait_second));
			cJSON_AddItemToObject(o, "tier_rule_wait_multiply_level",
								  cJSON_CreateString(queue->tier_rule_wait_multiply_level ? "true" : "false"));
			cJSON_AddItemToObject(o, "tier_rule_no_agent_no_wait",
								  cJSON_CreateString(queue->tier_rule_no_agent_no_wait ? "true" : "false"));
			cJSON_AddItemToObject(o, "discard_abandoned_after", cJSON_CreateNumber(queue->discard_abandoned_after));
			cJSON_AddItemToObject(o, "abandoned_resume_allowed",
								  cJSON_CreateString(queue->abandoned_resume_allowed ? "true" : "false"));
			cJSON_AddItemToObject(o, "max_wait_time", cJSON_CreateNumber(queue->max_wait_time));
			cJSON_AddItemToObject(o, "max_wait_time_with_no_agent",
								  cJSON_CreateNumber(queue->max_wait_time_with_no_agent));
			cJSON_AddItemToObject(o, "max_wait_time_with_no_agent_time_reached",
								  cJSON_CreateNumber(queue->max_wait_time_with_no_agent_time_reached));
			cJSON_AddItemToObject(o, "record_template", cJSON_CreateString(queue->record_template));
			cJSON_AddItemToObject(o, "skip_agents_with_external_calls",
								  cJSON_CreateString(queue->skip_agents_with_external_calls ? "true" : "false"));
	...
	if (!strcasecmp(arguments, "queue list agents")) {
		...
		cbt.row_process = 0;
		cbt.json_reply = cJSON_CreateArray();
		sql = switch_mprintf("SELECT agents.* FROM agents,tiers WHERE tiers.agent = agents.name AND tiers.queue = '%q'",
							 queue_name);
		cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_json_callback,
								&cbt /* Call back variables */);
		switch_safe_free(sql);
	...
	if (!strcasecmp(arguments, "queue list members")) {
		...
		sql = switch_mprintf(
			"SELECT  *,(%" SWITCH_TIME_T_FMT
			"-joined_epoch)+base_score+skill_score AS score FROM members WHERE queue = '%q' ORDER BY score DESC;",
			local_epoch_time_now(NULL), queue_name);
		cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_json_callback,
								&cbt /* Call back variables */);
		switch_safe_free(sql);
	...
	if (!strcasecmp(arguments, "member list")) {
		...
		cbt.json_reply = cJSON_CreateArray();
		sql = switch_mprintf("SELECT  *,(%" SWITCH_TIME_T_FMT
							 "-joined_epoch)+base_score+skill_score AS score FROM members ORDER BY score DESC;",
							 local_epoch_time_now(NULL));
		cc_execute_sql_callback(NULL /* queue */, NULL /* mutex */, sql, list_result_json_callback,
								&cbt /* Call back variables */);
		switch_safe_free(sql);
...
/* Macro expands to: switch_status_t mod_callcenter_load(switch_loadable_module_interface_t **module_interface,
 * switch_memory_pool_t *pool) */
SWITCH_MODULE_LOAD_FUNCTION(mod_callcenter_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	if (!AGENT_DISPATCH_THREAD_STARTED) { cc_agent_dispatch_thread_start(); }

	SWITCH_ADD_APP(app_interface, "callcenter", "CallCenter", CC_DESC, callcenter_function, CC_USAGE, SAF_NONE);
	SWITCH_ADD_APP(app_interface, "callcenter_track", "CallCenter Track Call",
				   "Track external mod_callcenter calls to avoid place new calls", callcenter_track, CC_USAGE,
				   SAF_SUPPORT_NOMEDIA);
	SWITCH_ADD_API(api_interface, "callcenter_config", "Config of callcenter", cc_config_api_function,
				   CC_CONFIG_API_SYNTAX);
	SWITCH_ADD_API(api_interface, "callcenter_break", "Stop watching an uuid and release agent", cc_break_api_function,
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_callcenter_shutdown)
{
	...
	switch_mutex_lock(globals.mutex);
	if (globals.running == 1) { globals.running = 0; }
	switch_mutex_unlock(globals.mutex);

	while (globals.threads) {
		switch_cond_next();
		if (++sanity >= 60000) { break; }
	}
